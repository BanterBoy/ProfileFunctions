...
.EXAMPLE
$IDs = 2,3,4,5,6,7,8,9,25,11,57,46,73,88,71,63
$IDs | ForEach-Object -Process { Get-FilteredEvents -ComputerName Server1,Server2,Server3,Server4,Server5,Server6 -LogName System -ID $_ -StartTime (Get-Date).AddDays(-21) -EndTime (Get-Date).AddDays(11) }
...
function Get-FilteredEvents {
    ...
    foreach ($ComputerName in $ComputerNames) {
        ...
        $events | Select-Object MachineName, TimeCreated, @{Name = "LogName"; Expression = { $LogName } }, ID, ProviderName, LevelDisplayName, Message
    }
}
