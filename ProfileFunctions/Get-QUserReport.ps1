function Get-QUserReport {
	<#
	.SYNOPSIS
	Function to extend the use of the QUser Command.

    .DESCRIPTION
	This function can be used to extend the use of the QUser Command in order to levy some addition automation.
	This command will query the Server/s specified and output the session details
    ...
    .EXAMPLE
	Get-QUserReport -ComputerName DANTOOINE
    ...
    .EXAMPLE
    Get-QUserReport -ComputerName DANTOOINE | Sort-Object IdleTime | Format-Table -AutoSize
...
#>
	...
	PROCESS {
		if ($PSCmdlet.ShouldProcess("$Computer", "Searching for sessions...")) {
